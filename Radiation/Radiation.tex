...
Після дещо громіздких обчислень з використанням (\ref{eq:magnetic_field}) отримуємо потужність випромінювання в тілесний кут \( do \) в напрямку~\(
\vect{n}_q \):
...
Перетворимо (\ref{eq:vector_potential_zero_order}), користуючись тотожністю \( \Div(x_j \vect{j}) = J_j + x_j \Div \vect{j} \) і
...
\end{equation*}
%
де враховано, що поза межами системи, що розглядається, \( \vect{j} = 0 \),  і на межі області, де струми відмінні від нуля, $\vect{j}\cdot d\vect{S} = 0$. Підстановка цього співвідношення в (\ref{eq:vector_potential_zero_order})
дає:
