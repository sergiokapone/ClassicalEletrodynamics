...
В цьому розділі розглянуто питання, що стосуються вільних електромагнітних полів у вакуумі,
тобто за відсутності зарядів та струмів. Центральним інструментом аналізу є розклад поля
за допомогою перетворення Фур'є, що дозволяє перейти від опису поля як функції часу до його
спектральних характеристик. Такий підхід природно веде до поняття плоских хвиль та
монохроматичних полів, для яких особливо зручним є комплексний формалізм при обчисленні
енергетичних співвідношень. Розділ завершується розкладом вільного поля на незалежні моди ---
гармонічні осцилятори, що є відправною точкою для квантової теорії електромагнітного поля.
...
Для функцій $f(t)$, $g(t)$ та їх образів Фур'є $\tilde{f}(\omega)$, $\tilde{g}(\omega)$ виконується рівність Парсеваля:
...
підсилювати чи послабляти певні ділянки спектра сигналу, що відповідають певній області частот. При цьому формальний запис гармонічних сигналів,
...
\begin{multline*}
	...
	= \frac{1}{\sqrt{2\pi}} \int\limits_{0}^{\infty} d\omega \left( \hat{\phi}(\omega) e^{-i\omega t} + \hat{\phi}^{*}(\omega) e^{i\omega t} \right)
\end{multline*}
...
\begin{equation}
	\phi(t) = \sqrt{\frac{2}{\pi}} \Re \int\limits_{0}^{\infty} d\omega e^{-i\omega t} \tilde{\phi}(\omega)
...
\begin{equation*}
	\int\limits_{-\infty}^{\infty} f^2(t) dt = \int\limits_{-\infty}^{\infty} |\tilde{f}(\omega)|^2 d\omega = 2\int\limits_{0}^{\infty} |\tilde{f}(\omega)|^2
	d\omega
\end{equation*}
...
\begin{multline*}
	\mathcal{E} = \int\limits_{-\infty}^{\infty} \oint\limits_{\partial\Omega} \vect{\Pi} \cdot d\vect{S}\, dt = \frac{c}{4\pi} \oint\limits_{\partial\Omega} \int\limits_{-\infty}^{\infty}
	\left[ \Efield \times \Bfield \right] \cdot dt\, d\vect{S} = \\
	= \frac{c}{4\pi} \oint\limits_{\partial\Omega} d\vect{S} \cdot \int\limits_{-\infty}^{\infty} \left[ \tilde{\Efield}(\omega) \times \tilde{\Bfield}^{*}(\omega) \right] d\omega
\end{multline*}
де \( \tilde{\Efield}(\omega), \tilde{\Bfield}(\omega) \) --- спектральні характеристики дійсних полів \( \Efield(t), \Bfield(t) \).

Аналогічно~\eqref{eq:conjugate_frequency}:

\begin{equation*}
	\tilde{\Efield}(-\omega) = \tilde{\Efield}^{*}(\omega), \quad \tilde{\Bfield}(-\omega) = \tilde{\Bfield}^{*}(\omega).
\end{equation*}

Тоді, розбиваючи інтеграл по \( d\omega \) на два додатки:

\begin{equation*}
	\int\limits_{-\infty}^{\infty} d\omega \left\{ \ldots \right\} = \int\limits_{0}^{\infty} d\omega \left\{ \ldots \right\} +
	\int\limits_{-\infty}^{0}
	d\omega
	\left\{ \ldots \right\},
\end{equation*}
...
\begin{equation}
	\mathcal{E} = \frac{c}{2\pi} \oint\limits_{\partial\Omega} d\vect{S} \cdot  \Re\int\limits_{0}^{\infty} \left[\tilde{\Efield}(\omega) \times \tilde{\Bfield}^{*}(\omega) \right] d\omega
...
\begin{equation*}
	\frac{d\mathcal{E}}{d\omega} = \frac{c}{2\pi} \oint\limits_{\partial\Omega} d\vect{S} \cdot \Re\left[ \tilde{\Efield}(\omega) \times \tilde{\Bfield}^{*}(\omega)\right], \quad \omega > 0
\end{equation*}

Тоді з~\eqref{eq:energy_positive_frequencies}:

\begin{equation*}
	\mathcal{E} = \int\limits_0^{\infty} \frac{d\mathcal{E}}{d\omega} d\omega
\end{equation*}
...
В попередніх співвідношеннях комплексні величини \(\tilde{\Efield}\), \(\tilde{\Bfield}\) --- це образи Фур'є відповідних дійсних полів. Доцільно розглядати також комплексні поля, які є розв'язками рівнянь Максвелла. Завдяки лінійності рівнянь, дійсні та уявні частини комплексних розв'язків також є розв'язками, але розрахунки з комплексними величинами часто є більш зручними. При розгляді ж спостережних величин або нелінійних (наприклад, енергетичних) співвідношень слід повернутися до дійсних частин розв'язків, які мають фізичний зміст.

Проілюструємо це на прикладі комплексних монохроматичних полів, тобто таких, що залежать від часу як \(e^{-i\omega t}\), \(\omega \neq 0\). Нехай
електричне та магнітне поля є монохроматичними, тобто
...
\begin{equation*}
	\Efield(t, \vect{r}) = \Efield_0(\vect{r}) e^{-i\alpha} e^{-i\omega t} \quad \Bfield(t, \vect{r}) = \Bfield_0(\vect{r}) e^{-i\alpha}
	e^{-i\omega t},
...
Вектор Пойнтінга густини потоку енергії виражаємо, як звичайно, через дійсні частини полів\footnote{%
	Дійсна частина комплексного числа $z$ є $\Re{(z)} = \frac{z + z^*}2$ (зірочка означає комплексне спряження).}:

\begin{multline*}
	\vect{\Pi} = \frac{c}{4\pi} \left[ \frac{\Efield + \Efield^{*}}{2} \times \frac{\Bfield + \Bfield^{*}}{2} \right] = \\
	= \frac{c}{8\pi} \left( \frac{[\Efield \times \Bfield^{*}]}2 + \frac{[\Efield^{*} \times \Bfield]}2 \right) + \frac{c}{16\pi} \left( [\Efield \times \Bfield] +
	[\Efield^{*} \times \Bfield^{*}] \right).
\end{multline*}

Складові другого доданку осцилюють як \(e^{\pm 2i\omega t}\); їх середнє за часом дорівнює нулю. Усереднюючи за часом, маємо:

\begin{equation}
	\langle \vect{\Pi} \rangle_t = \frac{c}{8\pi} \Re\left[\Efield \times \Bfield^{*}\right]
	\label{eq:time_averaged_poynting}
\end{equation}
де символ \(\langle F(t) \rangle_t = \lim\limits_{T \to \infty} \left\{ \frac{1}{T} \int\limits_{-T}^{T} F(t) dt \right\}\) означає усереднення за часом.
Фактично,~\eqref{eq:time_averaged_poynting} --- це вираз для середньої за часом густини потоку енергії через амплітуди монохроматичних електричного та магнітного
полів.
...
Зауважимо, що формули~\eqref{eq:time_averaged_poynting},~\eqref{eq:energy_density_average} не змінять свого вигляду у випадку суперпозиції монохроматичних
хвиль з різними частотами. Це очевидно, коли врахувати, що \(\langle e^{i(\omega_1 - \omega_2)t} \rangle_t = 0\), \(\omega_1 \neq \omega_2\).

Розглянемо \emph{монохроматичні плоскі хвилі}:

\begin{equation*}
	\Efield = \Efield_0 e^{i(\vect{k} \cdot \vect{r} - \omega t)}, \quad \Bfield = \Bfield_0 e^{i(\vect{k} \cdot \vect{r} - \omega t)}
	\label{eq:monochromatic_waves}
\end{equation*}

Підставимо це в рівняння Максвелла. За відсутності зарядів та струмів, маємо:
...
цих розв'язків зводиться до залежності від деякої змінної \(\zeta = (\vect{n} \cdot \vect{r})\), \(\vect{n}^2 = 1\). Тоді $\Efield = \Efield(\zeta, t)$, $\Bfield = \Bfield(\zeta, t)$.

Підставляючи в рівняння Максвелла, маємо:

\begin{equation}
	\Rot\Efield = \vect{n} \times \frac{\partial \Efield}{\partial \zeta } = -\frac{1}{c} \frac{\partial \Bfield}{\partial t}
	\label{eq:maxwell_rotE}
\end{equation}

\begin{equation}
	\Rot\Bfield = \vect{n} \times \frac{\partial \Bfield}{\partial\zeta} = \frac{1}{c} \frac{\partial \Efield}{\partial t}
	\label{eq:maxwell_rotB}
\end{equation}

\begin{equation}
	\Div\Efield = \vect{n} \cdot \frac{\partial \Efield}{\partial\zeta} = 0, \quad \Div\Bfield = \vect{n} \cdot
	\frac{\partial \Bfield}{\partial \zeta} = 0
...
Підставляючи сюди вираз для плоских хвиль \(\Efield = \Efield(\zeta, t)\):

\begin{equation*}
	\frac{\partial^2 \Efield}{\partial\zeta^2} = \frac{1}{c^2} \frac{\partial^2 \Efield}{\partial t^2}
\end{equation*}

Заміна \(U =\zeta - ct\), \(V =\zeta + ct\) приводить до рівняння:
...
Якщо \(\Efield = \Efield(\zeta, t)\), \(\Bfield = \Bfield(\zeta, t)\), з рівняння~\eqref{eq:maxwell_rotE} маємо:

\begin{equation*}
	\vect{n} \times \frac{\partial \Efield}{\partial \zeta} = \frac{\partial \Bfield}{\partial \zeta}
\end{equation*}
...
ансамбль хвильових полів --- і обчислення проводять за допомогою усереднень за ансамблем\footnote{Стаціонарні процеси, для яких часові середні
	...
	ситуації.}. Цей підхід добре відомий зі статистичної механіки. Його можна застосовувати також при обчисленні електричних та магнітних величин, що
...
розглянути аналогічно. Будемо користуватися комплексним формалізмом (див.~\ref{sec:spectral}); нагадаємо, що при цьому в виразі для густини енергії~\eqref{eq:energy_density_average}
фігурують квадрати модулів полів.
...
підрозділі ми позначаємо знаком \(\langle \dots \rangle\). З фізичного погляду процедуру усереднення можна інтерпретувати, як обчислення
...
Для стаціонарного процесу середнє \(\langle \phi(t) \rangle \equiv \langle \phi \rangle\). При розгляді випадкових полів випромінювання вважають \(\langle
\phi \rangle = 0\).

Важливою характеристикою випадкового процесу є кореляційна (автокореляційна) функція\refstepcounter{footnote}\footnote{У деяких підручниках у
	визначенні~\eqref{eq:autocorrelation} від правої частини віднімають \(\langle \phi(t) \rangle \langle \phi^{*}(t + \tau) \rangle\).}:
...
\begin{equation*}
	\tilde\phi_T(\omega) = \frac{1}{\sqrt{2\pi}} \int\limits_{-T}^{T} dt \, e^{i\omega t} \phi(t)
\end{equation*}

Обчислимо для стаціонарного процесу:

\begin{multline*}
	\langle \tilde\phi(\omega) \tilde\phi^{*}(\omega') \rangle = \frac{1}{2\pi} \int\limits_{-\infty}^{\infty} dt' \, e^{i(\omega - \omega')t'}
...
\begin{equation}
	\langle \tilde\phi(\omega) \tilde\phi^{*}(\omega') \rangle = \delta(\omega - \omega') S(\omega)
...
\begin{enumerate}
	\item \emph{Суперпозиція двох полів, що пов'язані умовою}:
	      ...
	      Умова зв'язку між \(\phi_1\) та \(\phi_2\) є випадком ідеальної корельованості, або \emph{когерентності} цих полів. Такі поля можна отримати при розділенні
	      світлового пучка в інтерферометрі, або, наприклад, у разі двох монохроматичних радіоджерел, що мають однакову частоту.

	\item Суперпозиція двох незалежних полів, генерованих двома незалежними випадковими випромінювачами:
	      ...
	      \emph{Випромінювання двох різних природних джерел світла є некогерентним}.

\end{enumerate}

Величину \(\Gamma = \langle \phi_1 \phi_2^{*} \rangle\), яку можна вимірювати за допомогою радіофізичних та оптичних пристроїв, називають \emph{функцією взаємної
	когерентності}\index{Функція!взаємної
...
\end{equation*}
%
називають \emph{ступенем взаємної когерентності}\index{Ступінь!взаємної когерентності}. Саме ця величина визначає контрастність смуг в інтерференційній картині від двох пучків світла.
...
\leqslant |\phi_1\|\phi_2|\), маємо \(|\gamma| \leqslant 1\).
...
бачити, що ці числа є додатними. Якщо матриця $\|\rho \|$ --- дійсна, її можна привести до діагонального виду  $\| \rho \| = \mathrm{diag}(\lambda_{\min}, \lambda_{\max})$ за допомогою просторових поворотів. Частку \(\lambda_{\min}/\lambda_{\max}\) називають \emph{коефіцієнтом деполяризації}.
...
Таким чином, кути \(\psi\) і \(\chi\) грають роль довготи й широти на сфері Пуанкаре. Граничні випадки: \(\chi = 0\) --- лінійна поляризація під кутом \(\psi\); \(\chi = \pm 45^{\circ}\) --- кругова.

%---------------------------------------------------------
\begin{figure}[ht!]\centering
	\input{\currfilebase/tikz/StokeParams.tikz}
...
%% --------------------------------------------------------
\section{Співвідношення між тривалістю сигналу та шириною його спектру}\label{sec:signal_spectrum}
%% --------------------------------------------------------

Міркування цього розділу застосовні до будь-яких сигналів чи хвильових
полів, що можна аналізувати за допомогою перетворення Фур'є, зокрема,
до хвильових пакетів\index{Хвильовий!пакет}, які є суперпозицією плоских хвиль з різними
частотами.

Нехай маємо деякий сигнал (чи хвильове поле у деякій фіксованій точці),
який описуємо за допомогою функції $\phi(t)$ (взагалі кажучи, комплексної) від
дійсної змінної $t$, причому для цієї функції існує перетворення Фур'є:
\begin{equation}\label{eq:Fourier}
	{\phi}(t) = \frac1{\sqrt{2\pi}} \int\limits_{-\infty}^{+\infty} \tilde\phi(\omega)\, e^{-i\omega t}\, d\omega.
\end{equation}

Вважатимемо, що $\phi(t)$ та $\tilde{\phi}(\omega)$ досить швидко спадають, якщо
відповідно $|t| \to \infty$ та $|\omega| \to \infty$.

Введемо величини, що характеризують тривалість сигналу $\Delta t$ та ширину
його спектру $\Delta\omega$. Середні значення фізичних величин, що відповідають
$\phi(t)$, обчислюватимемо з вагою $|\phi(t)|^2$ в $t$-просторі, а в
$\omega$-просторі~--- з вагою $|\tilde{\phi}(\omega)|^2$, причому, не зменшуючи
загальності, приймемо умову нормування\footnote{%
	Умова~\eqref{eq:norm} не зменшує загальності. Якщо її не накладати, усі
	міркування проводяться аналогічно, але у визначеннях середніх потрібно
	було б ділити на $\displaystyle\int\limits_{-\infty}^{+\infty}|\phi(t)|^2\,dt$.
}:
\begin{equation}\label{eq:norm}
	\int\limits_{-\infty}^{+\infty} |\phi(t)|^2\, dt = 1.
\end{equation}

Звідси, за рівністю Парсеваля~\eqref{eq:Parseval} маємо
\begin{equation*}
	\int\limits_{-\infty}^{+\infty} |{\phi}(t)|^2\, dt =  \int\limits_{-\infty}^{+\infty} |\tilde{\phi}(\omega)|^2\, d\omega = 1,
\end{equation*}
тобто спектр $\tilde{\phi}(\omega)$ також є нормованим.

Означимо середнє значення часу процесу:
\begin{equation*}
	\langle t \rangle = \int\limits_{-\infty}^{+\infty} t\, |\phi(t)|^2\, dt.
\end{equation*}
Зручно вибрати відлік часу таким чином, щоб $\langle t \rangle = 0$.

Природно оцінити тривалість процесу $\phi(t)$ через середньоквадратичне
відхилення $\sqrt{\langle t^2 \rangle}$, де
\begin{equation}\label{eq:Delta_t}
	\langle \Delta t^2 \rangle
	= \int\limits_{-\infty}^{+\infty} (t - \langle t \rangle)^2\, |\phi(t)|^2\, dt
\end{equation}

У $\omega$-просторі середнє значення частоти є:
\begin{equation*}
	\langle \omega \rangle = \int\limits_{-\infty}^{+\infty}
	\omega\, |\tilde{\phi}(\omega)|^2\, d\omega,
\end{equation*}
а ширину спектра частот опишемо величиною
\begin{equation}\label{eq:Delta_omega}
	\langle \Delta\omega^2 \rangle
	= \int\limits_{-\infty}^{+\infty}
	(\omega - \langle\omega\rangle)^2\, |\tilde{\phi}(\omega)|^2\, d\omega.
\end{equation}

Покажемо, що існує спільне обмеження на величини $\langle \Delta t^2 \rangle$ та $\langle \Delta\omega^2 \rangle$.
Для цього перепишемо~\eqref{eq:Delta_t} через перетворення Фур'є.

\begin{equation*}
	t \phi(t) = \frac{1}{\sqrt{2\pi}} \int d\omega \cdot i \left(\frac{d}{d\omega}e^{-i\omega t}\right) \tilde\phi(\omega) = \frac{1}{\sqrt{2\pi}} \int d\omega \cdot
	e^{-i\omega t} \left( -i \frac{d\tilde\phi}{d\omega} \right),
\end{equation*}
тобто помноження на \( t \) індукує диференціювання у просторі частот. Тоді формулу~\eqref{eq:Delta_t} можна переписати так:

\begin{equation}
	\langle \Delta t^2 \rangle =
	%
	\int dt \cdot t^2 |\phi(t)|^2  =
	%
	\int d\omega \left| \frac{d\tilde{\phi}}{d\omega} \right|^2
...
Звідси, після застосування усереднення, за допомогою~\eqref{eq:Delta_omega} та~\eqref{eq:time_duration_fourier}, маємо:
...
\begin{equation*}
	...
	\langle \Delta \omega^2 \rangle
	+
	x \int d\omega (\omega - \omega_a) \left[ \frac{d\tilde\phi}{d\omega}
		\tilde\phi^{*} + \frac{d\tilde\phi^{*}}{d\omega} \tilde\phi \right].
\end{equation*}
...
Це співвідношення має виконуватися для будь-яких $x$, тому дискримінант
квадратного тричлена має бути від'ємним. Звідси маємо нерівність:
\begin{equation}\label{eq:uncertainty}
	\langle \Delta t^2 \rangle \cdot \langle \Delta \omega^2 \rangle\geqslant \frac14,
\end{equation}
яку часто називають \emph{співвідношенням невизначеностей}\index{Співвідношення невизначеностей}\footnote{У квантовій механіці співвідношення~\eqref{eq:uncertainty} має аналог у вигляді зв'язку між тривалістю процесу та невизначеністю енергії. У випадку довжини пакета та розкиду хвильових векторів квантовий відповідник~\eqref{eq:uncertainty} --- співвідношення невизначеностей для імпульсу та координати.}.

Слід підкреслити, що співвідношення невизначеностей~\eqref{eq:uncertainty} було отримано для конкретних
оцінок~тривалості процесу~\eqref{eq:Delta_t} та ширини спектра частот~\eqref{eq:Delta_omega}. З~цього співвідношення
випливає, що сигнал, близький до монохроматичного, тобто такий, що має малий
розкид частот, має бути досить тривалим. Навпаки, короткочасний
сигнал має широкий спектр частот. Нерівність, аналогічну~\eqref{eq:uncertainty},
можна отримати для довжини хвильового пакета у певному напрямі та ширини
інтервалу відповідних хвильових векторів.
...
однорідною й описує вільні електромагнітні хвилі. \textit{Загальний розв'язок} \textit{неоднорідних рівнянь} Максвелла (за наявності зарядів та струмів)
...
\begin{equation}
	\vect{A}_{\vect{k}}(t) = \frac{1}{(2\pi)^{3/2}} \int d^3\vect{r} \cdot e^{-i\vect{k} \cdot \vect{r}} \vect{A}(t, \vect{r})
...
\begin{equation}
	\vect{A}(t, \vect{r}) = \frac{1}{(2\pi)^{3/2}} \int d^3\vect{k} \cdot e^{i\vect{k} \cdot \vect{r}} \vect{A}_{\vect{k}}(t)
...
\begin{equation*}
	\nabla^2 \vect{A}(t, \vect{r}) = \frac{1}{(2\pi)^{3/2}} \int d^3\vect{k} \, \nabla^2 \left( e^{i\vect{k} \cdot \vect{r}} \right) \vect{A}_{\vect{k}}(t) =
	\frac{1}{(2\pi)^{3/2}} \int d^3\vect{k} \, e^{i\vect{k} \cdot \vect{r}} (-k^2) \vect{A}_{\vect{k}}(t)
\end{equation*}
індукує домноження Фур'є-образу на \((-\vect{k}^2)\). Тому з рівняння~\eqref{eq:wave_equation_A} дістаємо:
...
\begin{equation}
	\vect{A}(t, \vect{r}) = \frac{1}{(2\pi)^{3/2}} \int d^3\vect{k} \, \left( \vect{C}_1(\vect{k}) e^{i\omega_{\vect{k}} t} + \vect{C}_2(\vect{k})
...
\begin{equation*}
	\vect{A}(t, \vect{r}) = \vect{A}^*(t, \vect{r}) = \frac{1}{(2\pi)^{3/2}} \int d^3\vect{k} \, \left( \vect{C}_1^*(\vect{k}) e^{-i\omega_{\vect{k}} t} +
...
Порівнюючи це з~\eqref{eq:general_solution_A} і беручи до уваги незалежність \(e^{-i\omega_{\vect{k}} t}\) та \(e^{i\omega_{\vect{k}} t}\),
отримаємо:
...
\begin{equation*}
	\int \vect{C}_1(\vect{k})
	e^{i(\omega_{\vect{k}} t +
			\vect{k} \cdot \vect{r})} d^3\vect{k}
	=
	\int \vect{C}_1(-\vect{k}) e^{i(\omega_{\vect{k}} t -  \vect{k} \cdot \vect{r})} d^3\vect{k}
	=
	\int \vect{C}_2^*(\vect{k}) e^{i(\omega_{\vect{k}} t -  \vect{k} \cdot \vect{r})}
	d^3\vect{k}.
\end{equation*}
...
\begin{equation}
	\vect{A}(t, \vect{r}) = \frac{1}{(2\pi)^{3/2}} \int \left( \vect{a}(\vect{k}) e^{-i(\omega_{\vect{k}} t - \vect{k} \cdot \vect{r})} +
	\vect{a}^*(\vect{k}) e^{i(\omega_{\vect{k}} t - \vect{k} \cdot \vect{r})} \right) d^3\vect{k}
	\label{eq:A_final_form}
\end{equation}

Формула~\eqref{eq:A_final_form} з умовою поперечності \(\vect{k} \cdot \vect{a}(\vect{k}) = 0\) подає загальний розв'язок хвильового рівняння~\eqref{eq:wave_equation_A} з умовою~\eqref{eq:div_A_zero}.

Доданок, що містить \(e^{-i(\omega_{\vect{k}} t - \vect{k} \cdot \vect{r})}\), називають додатно-частотною частиною поля, а комплексно-спряжений йому
доданок --- від'ємно-частотною частиною. Представлення~\eqref{eq:A_final_form} називають \emph{розбиттям на додатно-частотні та від'ємно-частотні компоненти}%
\footnote{%
Розбиття сигналу на додатно-частотні  та від'ємно-частотні  компоненти використовують у цифровій обробці сигналів.
}.
...
Кожному \(\vect{k}\) відповідає плоска хвиля \(\vect{a}(\vect{k}) e^{i(\omega_{\vect{k}} t - \vect{k} \cdot \vect{r})}\) --- частковий розв'язок
...
\begin{equation}
	\vect{A}(t, \vect{r}) = \frac{1}{(2\pi)^{3/2}} \int d^3\vect{k} \, \left( \vect{a}(\vect{k}) e^{-i\omega_{\vect{k}} t} + \vect{a}^*(-\vect{k})
	e^{i\omega_{\vect{k}} t} \right) e^{i\vect{k} \cdot \vect{r}}
...
\begin{equation}
	\Efield = -\frac{1}{c} \frac{\partial \vect{A}}{\partial t} = \frac{i}{(2\pi)^{3/2}} \int d^3\vect{k} \, \omega_{\vect{k}} \left( \vect{a}(\vect{k})
...
\begin{multline*}
	W_E =
	\frac{1}{8\pi} \int \Efield^2\ dV
	= \\ =
	\frac{1}{8\pi c^2} \int d^3\vect{k} \, \omega_{\vect{k}}^2 \left|
	\vect{a}(\vect{k})e^{-i\omega_{\vect{k}} t} -
	\vect{a}^*(-\vect{k})e^{i\omega_{\vect{k}} t}
	\right|^2
	= \\ =
	%
	\frac{1}{8\pi c^2} \int d^3\vect{k} \, \omega_{\vect{k}}^2 \left(
	\vect{a}^*(\vect{k}) \cdot \vect{a}(\vect{k}) -
	\vect{a}^*(-\vect{k}) \cdot \vect{a}(-\vect{k})
	\right.
	- \\ -
	%
	\left. 2\Re \left( \vect{a}(\vect{k}) \cdot \vect{a}(-\vect{k}) e^{-2i\omega_{\vect{k}} t} \right)
	\right)
	\label{eq:W_E}
\end{multline*}
...
\begin{equation*}
	\int d^3\vect{k}\ \omega^2_{\vect{k}} \, \vect{a}^*(-\vect{k}) \cdot \vect{a}(-\vect{k}) = \int d^3\vect{k} \ \omega^2_{\vect{k}}  \vect{a}^*(\vect{k}) \cdot \vect{a}(\vect{k}),
\end{equation*}
маємо:

\begin{equation}
	W_E = \frac{1}{4\pi c^2} \int d^3\vect{k} \, \omega_{\vect{k}}^2 \left\{ \vect{a}^*(\vect{k}) \cdot \vect{a}(\vect{k}) - \Re \left[ \vect{a}(\vect{k})
...
\begin{equation*}
	\Bfield = \Rot\vect{A} = \frac{1}{(2\pi)^{3/2}} \int d^3\vect{k} \, \left[ \vect{k} \times \left( \vect{a}(\vect{k}) e^{-i\omega_{\vect{k}} t} +
		\vect{a}^*(-\vect{k}) e^{i\omega_{\vect{k}} t} \right) \right] e^{i\vect{k} \cdot \vect{r}},
\end{equation*}
а енергія магнітного поля:

\begin{equation*}
	W_B = \frac{1}{8\pi} \int \Bfield^2 dV  = \frac{1}{8\pi} \int \left| \vect{k} \times \left( \vect{a}(\vect{k}) e^{-i\omega_{\vect{k}} t} +
	\vect{a}^*(-\vect{k}) e^{i\omega_{\vect{k}} t} \right) \right|^2 d^3\vect{k}
	\label{eq:W_B}
\end{equation*}
...
\begin{equation*}
	\left| \vect{k} \times \vect{a}(\vect{k}) \right|^2 = \vect{k}^2 \vect{a}^*(\vect{k}) \cdot \vect{a}(\vect{k}),
\end{equation*}
а також:

\begin{equation*}
	\left[ \vect{k} \times \vect{a}(\vect{k}) \right] \cdot \left[ \vect{k} \times \vect{a}(-\vect{k}) \right] = \vect{k}^2\  \vect{a}(\vect{k}) \cdot
	\vect{a}(-\vect{k}).
\end{equation*}
...
\begin{equation}
	W_B = \frac{1}{4\pi} \int d^3\vect{k} \, \vect{k}^2\  \left( \vect{a}^*(\vect{k}) \cdot \vect{a}(\vect{k}) + \Re \left[ \vect{a}(\vect{k}) \cdot
		\vect{a}(-\vect{k}) e^{-2i\omega_{\vect{k}} t} \right] \right)
	\label{eq:W_B_final}
\end{equation}

Оскільки \(\omega_{\vect{k}} = c|\vect{k}|\), загальна енергія вільного електромагнітного поля:

\begin{equation}
	W = W_E + W_B = \frac{1}{2\pi c^2} \int d^3\vect{k} \, \omega_{\vect{k}}^2 \vect{a}^*(\vect{k}) \cdot \vect{a}(\vect{k})
...
За допомогою цих співвідношень отримаємо такий вираз для загальної енергії поля~\eqref{eq:total_energy}:
\begin{equation}
	W = \frac12 \int d^3\vect{k} \left( \vect{P}^2_{\vect{k}} + \omega^2_{\vect{k}}\vect{Q}^2_{\vect{k}}\right),
\end{equation}
що має вигляд гамільтоніана континуальної системи осциляторів з частотою $\omega_{\vect{k}}$. Цю формулу називають \emph{розкладом поля на осцилятори}. Зауважимо, що для кожного $\vect{k}$:
\begin{equation*}
	\dot{\vect{Q}}_{\vect{k}} = \vect{P}_{\vect{k}}, \quad
	\dot{\vect{P}}_{\vect{k}} = - \omega^2_{\vect{k}} \vect{Q}_{\vect{k}}.
\end{equation*}
що випливає як з гамільтонової форми рівнянь руху відповідно до~\eqref{eq:total_energy}, так і безпосередньо з рівнянь~\eqref{eq:Pk},~\eqref{eq:Qk}. <<Координати>> задовольняють рівнянням:
\begin{equation*}
	\ddot{\vect{Q}}_{\vect{k}} + \omega^2_{\vect{k}}\vect{Q}_{\vect{k}} = 0
\end{equation*}
та описують коливання поля з частотою $\omega_{\vect{k}}$%
\footnote{%
Ці рівняння повністю збігаються з рівняннями гармонічного осцилятора з класичної механіки:
\[
\dot{q} = p,\quad \dot{p} = -\omega^2 q,\quad \ddot{q} + \omega^2 q = 0.
\]
}.
%\footnote{
%Ці рівняння повністю збігаються з рівняннями гармонічного осцилятора з класичної механіки:
...
%	\dot{q} = p, \quad \dot{p} = -\omega^2 q, \quad \ddot{q} + \omega^2 q = 0.
%\]
%Таким чином, кожна мода поля з хвильовим вектором $\vect{k}$ поводиться як незалежний гармонічний осцилятор з частотою $\omega_{\vect{k}}$, де роль узагальненої координати та імпульсу відіграють $\vect{Q}_{\vect{k}}$ та $\vect{P}_{\vect{k}}$ відповідно.
%}.


Це означає, що, кожна мода поля з хвильовим вектором $\vect{k}$ поводиться як незалежний гармонічний осцилятор з частотою $\omega_{\vect{k}}$. Хоча таке представлення електромагнітного поля в класичній електродинаміці видається не більш ніж зручним математичним описом, але воно має принципове значення при переході до квантової теорії. Поле набуває вигляду суми незалежних осциляторів, що дозволяє застосувати стандартну процедуру квантування, відпрацьовану для систем зі скінченним числом ступеней свободи.

%Розклад поля на осцилятори та гамільтонова форма рівнянь важливий для побудови квантової теорії випромінювання. В стандартній процедурі квантування гамільтонові змінні стають операторами і для них вводять певні комутаційні співвідношення.
...
У хвильовому полі
\[
	\phi(t, \vect{r}) = \int d^3 \vect{k} \, \Phi(\vect{k}) e^{i(\vect{k} \cdot \vect{r} - \omega(\vect{k}) t)}
\]
закон дисперсії заданий рівнянням
\[
	\omega(\vect{k}) = \omega_0 + \vect{V} \cdot (\vect{k} - \vect{k}_0).
\]
Покажіть, що модуль поля має вид
\[
	|\phi(t, \vect{r})| = F(\vect{r} - \vect{V}t).
\]
\end{problem}

\begin{problem}
Знайти умову на поле \( \varphi(t) \) (див. п.~\ref{sec:signal_spectrum}), коли співвідношення невизначеностей~\eqref{eq:uncertainty} перетворюється на точну рівність:
\[
	\left\langle \Delta t^2 \right\rangle \left\langle \Delta \omega^2 \right\rangle = \frac14.
\]
\end{problem}
