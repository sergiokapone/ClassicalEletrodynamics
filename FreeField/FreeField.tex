...
Очевидно, усі ці співвідношення зберігаються при усередненні за ансамблем :
\begin{equation*}
    \langle F \rangle = \int dt \cdot F(t) |\phi(t)|^2,\ \langle \tilde{F} \rangle = \int d\omega \cdot \tilde{F}(\omega) |\phi(\omega)|^2.
\end{equation*}
...
\begin{equation}
\langle \Delta t^2 \rangle_{\phi} = \int dt \cdot t^2 \langle |\phi(t)|^2 \rangle
...
\end{equation}
що має вигляд гамільтоніану континуальної системи осциляторів з частотою $\omega_{\vect{k}}$. Цю формулу називають розкладом поля на осцилятори. Зауважимо, що для кожного $\vect{k}$:
\begin{equation*}
...
\begin{problem}
    У хвильовому полі
    \[
    \varphi(t, \vect{r}) = \int d^3 \vect{k} \, \Phi(\mathbf{k}) e^{i(\vect{k} \cdot \vect{r} - \omega(\vect{k}) t)}
    \]
    закон дисперсії заданий рівнянням
    \[
    \omega(\vect{k}) = \omega_0 + \mathbf{V} \cdot (\vect{k} - \vect{k}_0).
    \]
    Покажіть, що модуль поля має вид
    \[
    |\varphi(t, \mathbf{r})| = F(\vect{r} - \vect{V}t).
    \]
\end{problem}

\begin{problem}
    Знайти умову на поле \( \varphi(t) \) (див. п.~\ref{sec:uncert_principle}), коли співвідношення невизначеностей \eqref{eq:uncertainty_relation} перетворюється на точну рівність:
    \[
    \left\langle \Delta t^2 \right\rangle_{\varphi} \left\langle \Delta \omega^2 \right\rangle_{\varphi} = \frac14.
    \]
\end{problem}
