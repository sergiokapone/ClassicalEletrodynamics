...
%
%Узагальнена функція --- це лінійний неперервний функціонал, означений на \( \mathbb{D} \). Множину таких узагальнених функцій позначають через \( \mathbb{D}' \).
%
...
Нехай $\hat{L}$ --- лінійний диференціальний оператор скінченного порядку зі сталими коефіцієнтами, $x \in \mathbb{R}^n$.

Розглянемо неоднорідне рівняння
\begin{equation}\label{eq:inhomogeneous}
    \hat{L} \phi(x) = f(x),
\end{equation}
де $f$ --- задана узагальнена функція.

\emph{Фундаментальним розв'язком} оператора $\hat{L}$ називається узагальнена функція $G$, що задовольняє
\begin{equation}\label{eq:fundamental_solution}
    \hat{L} G = \delta(x).
\end{equation}

Фізично $G(x)$ --- це відгук системи на точкове джерело, вміщене в початок координат.

Якщо $G$ --- фундаментальний розв'язок оператора $\hat{L}$, то розв'язок рівняння \eqref{eq:inhomogeneous} дається згорткою
\begin{equation}\label{eq:solution_via_convolution}
    \phi(x) = (G * f)(x) \equiv \int_{\mathbb{R}^n} G(y) f(x - y) \, dy,
\end{equation}
за умови, що згортка існує (наприклад, коли $f$ фінітна або достатньо швидко спадає).

Використовуючи сталість коефіцієнтів оператора $\hat{L}$, маємо
\begin{equation*}
        \hat{L} (G * f) = (\hat{L} G) * f = \delta * f = f.
\end{equation*}


\medskip
Наведемо фундаментальні розв'язки для основних операторів математичної фізики.

\begin{enumerate}
    \item \textbf{Оператор Лапласа} в $\mathbb{R}^3$ ($\vect{r} \in \mathbb{R}^3$, $r = |\vect{r}|$):
    \begin{equation}\label{eq:laplace_fundamental}
        G(\vect{r}) = -\frac{1}{4\pi r}, \qquad \Laplasian G = \delta(\vect{r}).
    \end{equation}

    \item \textbf{Оператор Даламбера} (хвильовий оператор, $c=1$):
    \[
        \Dalambertian = \frac{\partial^2}{\partial t^2} - \Laplasian, \qquad x = (t, \vect{r}) \in \mathbb{R}^4.
    \]
    Запізнювальна функція Гріна:
    \begin{equation}\label{eq:dalambert_fundamental}
        G(x) = \frac{1}{4\pi r}\,\delta(t - r), \qquad \Dalambertian G = \delta(x).
    \end{equation}

    \item \textbf{Оператор Гельмгольца}: $\hat{L} = \Laplasian + k^2$, $k \in \mathbb{R}\setminus\{0\}$.
    Випромінювальна функція Гріна:
    \begin{equation}\label{eq:helmholtz_fundamental}
        G(\vect{r}) = -\frac{e^{ikr}}{4\pi r}, \qquad (\Laplasian + k^2) G = \delta(\vect{r}).
    \end{equation}
    У границі $k \to 0$ вираз переходить у фундаментальний розв'язок оператора Лапласа.
\end{enumerate}

\medskip
Підставляючи ці $G$ у \eqref{eq:solution_via_convolution}, отримуємо розв'язки відповідних рівнянь фізики (в гаусовій системі одиниць):

\begin{align}
    \Laplasian \phi &= -4\pi \rho \quad &\Longrightarrow& \quad \phi(\vect{r}) = \int \frac{\rho(\vect{r}')}{|\vect{r} - \vect{r}'|} \, dV', \\[4pt]
    \Dalambertian \phi &= 4\pi \rho \quad &\Longrightarrow& \quad \phi(t, \vect{r}) = \int \frac{\rho(t - |\vect{r} - \vect{r}'|, \vect{r}')}{|\vect{r} - \vect{r}'|} \, dV', \\[4pt]
    (\Laplasian + k^2)\phi &= -4\pi \rho \quad &\Longrightarrow& \quad \phi(\vect{r}) = \int \frac{e^{ik|\vect{r} - \vect{r}'|}}{|\vect{r} - \vect{r}'|}\,\rho(\vect{r}')\, dV'.
\end{align}
