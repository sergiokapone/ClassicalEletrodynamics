...
З формули для перетворення густини заряду~\eqref{eq:charge_density_transform}, яку легко узагальнити на випадок довільного напрямку швидкості, випливає, що величина $\rho\sqrt{1-\vect{v}^{2}/c^{2}} = \rho\,ds/dt$ є інваріантом --- це густина заряду у власній системі елемента середовища. Тоді $j^{\mu}$ має ті ж трансформаційні властивості, що й $dx^{\mu}/ds$, тобто це контраваріантний вектор. Його називають \textbf{чотиривектором густини струму\index{Чотиривектор!густини струму}}. Цей вектор введено для середовища з неперервним розподілом зарядів та швидкостей, але його можна означити та для точкових зарядів $q_{i}$ з траєкторіями $x^{\alpha}=x^{\alpha}_{(i)}$ (індекс $i=1,2,\ldots$ нумерує заряди):
\begin{equation*}
    j^{\mu} = c\sum_{i}q_{i}\int ds_{i}\,\delta^{(4)}(x - x_{(i)}(s_{i}))
    \frac{dx_{(i)}^{\mu}}{ds_{i}},
\end{equation*}
де інтегрування проводиться вздовж траєкторій зарядів, $\delta^{(4)}(x)$ --- чотиривимірна дельта-функція Дірака\footnote{За визначенням, для будь-якої неперервної функції $f$: $\int\delta^{(4)}(x-x_0)f(x)\,d^4\vect{x} = f(x_0)$.}.

Для опису електромагнітного поля введемо антисиметричну матрицю
\begin{equation*}
\left\| F_{\mu\nu} \right\| =
\begin{pmatrix}
0 & E_1 & E_2 & E_3 \\
...
\end{pmatrix},
\end{equation*}
де $F_{\mu\nu}=-F_{\nu\mu}$; $E_i$ --- компоненти напруженості електричного поля\index{Напруженість електричного поля}, $B_i$ --- індукції магнітного поля в декартових координатах $x^{i}$ ($i=1,2,3$). Цю матрицю називають \emph{тензором електромагнітного поля\index{Тензор!електромагнітного поля}}. Компоненти цього тензора можна означити ще й так:
\begin{equation*}
...
\textbf{Рівняння Максвелла} в гаусовій системі одиниць\index{Гаусова система одиниць}, переписані за допомогою $F$, мають вид:
...
\begin{equation}\label{eq:charge_conservation}
    \partial_{\mu}j^{\mu} = 0,
\end{equation}
як необхідна умова існування розв'язку рівнянь електромагнітного поля. Оскільки $j^{\mu}$ є 4-вектор, а $\partial_{\alpha}j^{\beta}$ --- тензор, згортка $\partial_{\mu}j^{\mu}$ є інваріантом. Рівняння~\eqref{eq:charge_conservation} є інваріантним при перетвореннях Лоренца.
...
\[
    ...
    + \frac{1}{c}\frac{\partial\Efield}{\partial t},
\]
...
\[
    \Rot\Efield = -\frac{1}{c}\frac{\partial\Bfield}{\partial t}.
\]
\end{problem}
...
Досі ми не з'ясовували трансформаційні властивості величин $F_{\mu\nu}$, які є розв'язками рівнянь електродинаміки~\eqref{eq:Maxwell_4D_1},~\eqref{eq:Maxwell_4D_2}. За певних граничних умов розв'язок цих рівнянь визначений єдиним чином. Для подальшого досить розглянути якусь одну постановку задачі для~\eqref{eq:Maxwell_4D_1},~\eqref{eq:Maxwell_4D_2}, що забезпечує єдиність, зокрема, що відповідає розгляду ізольованої системи. Адже фізичні властивості поля, в тому числі та трансформаційні співвідношення, що пов'язують різні інерціальні системи, не залежать від того, яка конфігурація зарядів створює це поле.
...
Відносно інерціальної системи відліку $S$ з координатами $\{t,\vect{r}\}$, $\vect{r}=(x,y,z)$, рухається зі сталою швидкістю $\vect{v}$ інша інерціальна система $S'$ з координатами $\{t',\vect{r}'\}$, $\vect{r}'=(x',y',z')$. У деякий момент початки координат збігалися. Перетворення координат $S\to S'$ мають вид
\begin{equation*}
...
\begin{align*}
    ...
    \qquad
    \Bfield'_{\|} = \Bfield_{\|}, \\
    \Efield'_{\bot} &= \frac{\Efield_{\bot}
    + \tfrac{1}{c}[\vect{v}\times\Bfield]}{\sqrt{1-\beta^2}},
    \qquad
    \Bfield'_{\bot} = \frac{\Bfield_{\bot}
    - \tfrac{1}{c}[\vect{v}\times\Efield]}{\sqrt{1-\beta^2}}.
\end{align*}
\end{problem}

%=========================================================
\begin{problem}
За допомогою формул перетворення електричного та магнітного полів при переході $S\to S'$ з вправи~\ref{prb:3.3} покажіть прямим обчисленням, що величини $\Efield^{2}-\Bfield^{2}$ та $\Efield\cdot\Bfield$ при цьому переході залишаються незмінними.

\vspace{1ex}
\textit{Розв'язок}:
\begin{itemize}
\item Інваріантність $\Efield^2 - \Bfield^2$.

Розкладемо на паралельну та перпендикулярну до швидкості частини:
\begin{equation*}
    \Efield^2 - \Bfield^2 = (E_\parallel^2 + E_\perp^2) - (B_\parallel^2 + B_\perp^2).
\end{equation*}

У системі $S'$:
\begin{equation*}
    E_\parallel'^2 - B_\parallel'^2 = E_\parallel^2 - B_\parallel^2.
\end{equation*}

Для перпендикулярних компонент візьмемо
\begin{equation*}
    \Efield_\perp = (0, E_y, E_z), \quad \Bfield_\perp = (0, B_y, B_z), \quad \vect{\beta} = (\beta, 0, 0),
\end{equation*}
тоді
\begin{equation*}
    [\vect{\beta}\times \Bfield_\perp] = (0, -\beta B_z, \beta B_y), \quad
    [\vect{\beta}\times \Efield_\perp] = (0, -\beta E_z, \beta E_y).
\end{equation*}

Отже,
\begin{equation*}
    \begin{aligned}
    E'_{y'} &= \gamma (E_y + \beta B_z), \quad E'_{z'} = \gamma (E_z - \beta B_y), \\
    B'_{y'} &= \gamma (B_y - \beta E_z), \quad B'_{z'} = \gamma (B_z + \beta E_y).
    \end{aligned}
\end{equation*}

Обчислимо:
\begin{align*}
    E_\perp'^2 = \gamma^2 \left[ (E_y + \beta B_z)^2 + (E_z - \beta B_y)^2 \right], \\
    B_\perp'^2 = \gamma^2 \left[ (B_y - \beta E_z)^2 + (B_z + \beta E_y)^2 \right].
\end{align*}

Різниця:
\begin{equation*}
    E_\perp'^2 - B_\perp'^2 = \gamma^2 \bigl[ E_\perp^2 + 2\beta X + \beta^2 B_\perp^2 - B_\perp^2 - 2\beta X - \beta^2 E_\perp^2 \bigr],
\end{equation*}
де \(X = E_y B_z - E_z B_y\). Члени з \(X\) скорочуються, тому
\begin{equation*}
    E_\perp'^2 - B_\perp'^2 = \gamma^2 (1 - \beta^2)(E_\perp^2 - B_\perp^2) = E_\perp^2 - B_\perp^2.
\end{equation*}

Додаючи паралельну частину, отримуємо
\begin{equation*}
    \Efield'^2 - \Bfield'^2 = \Efield^2 - \Bfield^2.
\end{equation*}

\item Інваріантність $\Efield \cdot \Bfield$.

Скалярний добуток:
\begin{equation*}
    \Efield' \cdot \Bfield' = E_\parallel' B_\parallel' + \Efield_\perp' \cdot \Bfield_\perp'.
\end{equation*}

Паралельна частина: \(E_\parallel B_\parallel\) інваріантна. Для перпендикулярної:
\begin{equation*}
    \Efield_\perp' \cdot \Bfield_\perp' = E_y' B_y' + E_z' B_z'.
\end{equation*}

Підставляємо вирази для \(E_y', B_y', E_z', B_z'\):
\begin{equation*}
    \begin{aligned}
    E_y' B_y' &= \gamma^2 (E_y + \beta B_z)(B_y - \beta E_z), \\
    E_z' B_z' &= \gamma^2 (E_z - \beta B_y)(B_z + \beta E_y).
    \end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    E_y' B_y' = \gamma^2 \left( E_y B_y - \beta E_y E_z + \beta B_z B_y - \beta^2 B_z E_z \right), \\
    E_z' B_z' = \gamma^2 \left( E_z B_z + \beta E_z E_y - \beta B_y B_z - \beta^2 B_y E_y \right).
\end{aligned}
\end{equation*}

Додаємо:
\begin{equation*}
    \gamma^2 \bigl[ E_y B_y + E_z B_z - \beta^2 (E_y B_y + E_z B_z) \bigr] = \gamma^2 (1 - \beta^2) (\Efield_\perp \cdot \Bfield_\perp) = \Efield_\perp \cdot \Bfield_\perp.
\end{equation*}

Отже,
\begin{equation*}
    \Efield' \cdot \Bfield' = E_\parallel B_\parallel + \Efield_\perp \cdot \Bfield_\perp = \Efield \cdot \Bfield.
\end{equation*}
...
Отже:
\begin{equation*}
    \Efield'^2 - \Bfield'^2 = \Efield^2 - \Bfield^2, \qquad
    \Efield' \cdot \Bfield' = \Efield \cdot \Bfield.
\end{equation*}
...
\begin{equation*}
    m\frac{d\vect{v}}{dt} = q\!\left(\Efield
    + \frac{1}{c}[\vect{v}\times\Bfield]\right).
\end{equation*}
Ми далі використаємо навіть слабшу умову, що класичні рівняння руху виконуються при $\vect{v}=0$. Основна ідея полягає у тому, що співвідношення між тензорними величинами, записані в одній системі відліку, можна поширити на будь-які системи.
...
Зафіксуємо деякий момент $t_1$ і подію~А з координатами $(t_1,\vect{r}(t_1))$ та розглянемо інерціальну систему відліку $S_1$, таку, де швидкість частинки у цей момент дорівнює нулю: $\vect{v}=0$. Тоді, згідно з принципом відповідності, в цій системі у момент події~А можна використати ньютонівські співвідношення (в декартових координатах):
\begin{equation*}
    m\frac{d\vect{v}}{dt} = q\!\left(\Efield
    + \frac{1}{c}[\vect{v}\times\Bfield]\right) = q\Efield,
    \qquad
    m\frac{d(\vect{v}^2/2)}{dt} = 0.
\end{equation*}
Завдяки цьому легко обчислити усі компоненти в $S_1$: $R^{\mu}=0$ в момент події~$A$. Але, оскільки, за побудовою, $R^{\mu}$ це 4-вектор, згідно з властивостями векторів він дорівнює нулю у довільній інерціальній системі. Звідси маємо рівняння в момент події~А:
\begin{equation}\label{eq:rel_motion}
    mc^2\frac{du^{\mu}}{ds} = qF^{\mu}{}_{\nu}\,u^{\nu}.
\end{equation}
Ці міркування можна повторити для будь-якої події на траєкторії частинки, тому \textbf{рівняння~\eqref{eq:rel_motion} є шуканим релятивістським узагальненням рівняння руху зарядженої частинки}.

Звернемо увагу, що серед чотирьох рівнянь~\eqref{eq:rel_motion} лише три є незалежними, якщо врахувати зв'язок між компонентами 4-швидкості. Навпаки, можна показати, що вони не суперечать цьому зв'язку: якщо помножити~\eqref{eq:rel_motion} на $u_{\mu}$, через антисиметрію $F^{\mu\nu}$ у правій частині маємо $F^{\mu}{}_{\nu}\,u^{\nu}u_{\mu} \equiv F^{\mu\nu}u_{\mu}u_{\nu} \equiv 0$, звідки $u_{\mu}\,\frac{du^{\mu}}{ds} = \frac12\,\frac{d(u_{\mu}u^{\mu})}{ds} = 0$, тобто з рівнянь~\eqref{eq:rel_motion} випливає $u^{\mu}u_{\mu}=\const$. Кількість ступенів вільності, що визначається кількістю незалежних початкових умов для рівнянь руху, тут така ж сама, як і в класичній механіці.
...
\begin{equation*}
    ...
    = q\!\left(\Efield + \frac{1}{c}[\vect{v}\times\Bfield]\right).
\end{equation*}
...
%=========================================================
\begin{problem}
Частинка з зарядом $q$ та масою $m$ рухається в однорідному сталому електромагнітному полі, причому електричне $\Efield$ та магнітне $\Bfield$ поля паралельні. В початковий момент $t=0$ частинка була у початку координат та мала швидкість $v$, перпендикулярну до полів. Знайти залежності координат зарядженої частинки від власного часу $\tau$.

\textit{Відповідь}. Нехай вісь $OZ$ спрямована вздовж вектора напруженості електричного поля, а вісь $OX$ --- вздовж напрямку швидкості в початковий момент. Тоді
\begin{align*}
    ct &= \frac{c\gamma}{\omega_E}\sh(\omega_E\tau),
    &z &= \frac{c\gamma}{\omega_E}\!\left(\ch(\omega_E\tau)-1\right), \\
    x &= \frac{v\gamma}{\omega_B}\sin(\omega_B\tau),
    &y &= \frac{v\gamma}{\omega_B}\!\left(\cos(\omega_B\tau)-1\right),
\end{align*}
де $\omega_E = \dfrac{qE}{mc}$, $\omega_B = \dfrac{qB}{mc}$, $\gamma = \dfrac{1}{\sqrt{1-v^2/c^2}}$.
\end{problem}
...
\begin{equation*}
    F_{\mu\nu}(x) = f_{\mu\nu}\exp(-i\Omega),
\end{equation*}
де компоненти тензора $f_{\mu\nu}$ --- сталі, а скаляр $\Omega$ --- лінійна функція координат, $\Omega(x)=k_{\mu}x^{\mu}\equiv k^0x^0 - \vect{k}\cdot\vect{r}$; $k_{\mu}=\partial_{\mu}\Omega$ --- хвильовий (коваріантний) чотиривектор\index{Хвильовий!вектор}.

Частота коливань періодичного процесу, яка сприймається спостерігачем на світовій лінії $x_\text{obs}^{\mu}(\tau)$, --- це швидкість зміни фази $\Omega\!\left(x_{\text{obs}}^{\mu}(\tau)\right)$ за власним часом спостерігача:
...
\end{equation}
де $u_{\text{obs}}^{\mu}=dx_{\text{obs}}^{\mu}/ds_{\text{obs}}$ --- 4-швидкість спостерігача, $\tau$ --- власний час спостерігача, що вимірює частоту, $s=c\tau=s_{\text{obs}}$.

Вектор $K^{\mu}=k^{\mu}-k_{\alpha}u_{\text{obs}}^{\alpha}\,u_{\text{obs}}^{\mu}$, ортогональний до $u_{\text{obs}}^{\mu}$, визначає напрямок руху електромагнітних хвиль відносно спостерігача. Дійсно, у власній системі спостерігача 4-швидкість має лише одну ненульову компоненту $\{u_{\text{obs}}^{\mu}\}=\{1,0,0,0\}$, звідки $K^0=0$, а тривимірні компоненти векторів збігаються з $K^i=k^i$.
...
Згортаючи~\eqref{eq:Bianchi_plane} з $f^{\alpha\beta}$ із врахуванням~\eqref{eq:transversality} дістанемо $f^{\alpha\beta}f_{\alpha\beta}=0$, або $F^{\alpha\beta}F_{\alpha\beta}=0$, що еквівалентне $\Efield^2=\Bfield^2$. Згорнемо~\eqref{eq:Bianchi_plane} з $\varepsilon^{\mu\nu\alpha\beta}f_{\mu\nu}$ та врахуємо, що~\eqref{eq:Bianchi_plane} можна записати як $\varepsilon^{\mu\alpha\beta\gamma}k_{\alpha}f_{\beta\gamma}=0$. Після такої операції зникають доданки
\begin{equation*}
...
\end{equation*}
і залишається $\varepsilon^{\mu\nu\alpha\beta}f_{\mu\nu}f_{\alpha\beta}=0$, або $\varepsilon^{\mu\nu\alpha\beta}F_{\mu\nu}F_{\alpha\beta}=0$, що еквівалентне $\Efield\cdot\Bfield=0$. Як і слід було очікувати, отримано звичайні співвідношення для плоских хвиль у вакуумі.

%% --------------------------------------------------------
\section{Ефект Доплера}
%% --------------------------------------------------------

Отримаємо формулу ефекту Доплера, який полягає у зміні спостережуваної частоти випромінювання залежно від руху джерела відносно спостерігача. Аналогічно~\eqref{eq:freq_observer} власна частота коливань випромінювача з траєкторією $x_s^{\mu}(\tau_s)$, власним часом $\tau_s$ та 4-швидкістю $u_s^{\mu}=dx_s^{\mu}/ds_s$ є $\omega_0 = (d/d\tau_s)\,\Omega(x_s^{\mu}(\tau_s)) = ck_{\mu}u_s^{\mu}$. Ця формула інваріантна, її можна використовувати у будь-якій системі.

Далі усі величини розглянемо у системі спостерігача, де його 4-швидкість $u_{\text{obs}}^{\mu}=\{1,0,0,0\}$, а виміряна ним частота $\omega_{\text{obs}}=ck_{\mu}u_{\text{obs}}^{\mu}=ck_0=ck^0$. Напрям поширення хвиль, що реєструє спостерігач, збігається з напрямком
від джерела до спостерігача $\vect{n}=\vect{k}/|\vect{k}|=\vect{k}/k^0$.

Після переходу в систему спостерігача маємо
\begin{equation}\label{eq:Dopler}
    \frac{\omega_{\text{obs}}}{\omega_0}
    = \frac{k_{\mu}u_{\text{obs}}^{\mu}}{k_{\mu}u_0^{\mu}}
    = \frac{k_0}{k_{\mu}u_0^{\mu}}
    = \frac{1}{u_0^0(1+\vect{n}\cdot\vect{v}/c)}
    = \frac{\sqrt{1-\vect{v}^2/c^2}}{1-\vect{n}\cdot\vect{v}/c},
\end{equation}
...
\begin{problem}%
    Частинка (джерело світла) рухається з постійною швидкістю $\vect{v}$ відносно спостерігача. У власній системі вона випромінює монохроматичну світлову хвилю з частотою $\omega_0$ у напрямку, що утворює кут $\theta_0$ з віссю $x_0$ (вісь, вздовж якої напрямлено $\vect{v}$).
    Знайти в системі відліку спостерігача:
    \begin{itemize}
        \item кут $\theta$ між напрямком поширення хвилі та віссю $x$ (релятивістська аберація);
        \item при якому куті $\theta_0$ у власній системі частота $\omega$, що реєструється спостерігачем, дорівнює власній частоті $\omega_0$;
        \item як співвідносяться $\theta$ і $\theta_0$ у нерелятивістській границі $\beta\ll 1$.
    \end{itemize}
    \vspace*{1ex}
    \textit{Розв'язок}: Запишемо 4-вектор хвилі у власній системі джерела:
    \[
        k_0^\mu = \frac{\omega_0}{c}\left(1,\, \cos\theta_0,\, \sin\theta_0,\, 0\right).
    \]
    Перетворення Лоренца вздовж осі $x$ ($\beta = v/c$, $\gamma = 1/\sqrt{1-\beta^2}$) дає часову компоненту
    \[
        \frac{\omega}{c} = \gamma\frac{\omega_0}{c}(1+\beta\cos\theta_0),
    \]
    що збігається з формулою~\eqref{eq:Dopler} при $\vect{n}\cdot\vect{v}/c = \beta\cos\theta_0$ (де $\vect{n}=\vect{k}/|\vect{k}|$ --- напрямок поширення хвилі у системі спостерігача). Для просторових компонент:
    \[
        k_x = \frac{\omega}{c}\cos\theta = \gamma\frac{\omega_0}{c}(\cos\theta_0+\beta), \qquad
        k_y = \frac{\omega}{c}\sin\theta = \frac{\omega_0}{c}\sin\theta_0.
    \]
    Поділивши $k_y$ на $k_x$, отримуємо \emph{формулу аберації}:
    \[
        \tan\theta = \frac{\sin\theta_0}{\gamma(\cos\theta_0+\beta)}.
    \]

    Умова $\omega = \omega_0$ з формули Доплера:
    \[
        \gamma(1+\beta\cos\theta_0) = 1 \implies \cos\theta_0 = -\frac{\gamma-1}{\gamma\beta} = -\frac{\beta}{1+\sqrt{1-\beta^2}}.
    \]
    Оскільки $\cos\theta_0 < 0$, цей кут лежить у другій чверті: джерело має випромінювати дещо назад у власній системі, щоб компенсувати ефект Доплера.

    При $\beta\ll 1$, $\gamma\approx 1$:
    \[
        \tan\theta \approx \frac{\sin\theta_0}{\cos\theta_0+\beta} \approx \tan(\theta_0 - \beta\sin\theta_0),
    \]
    тобто $\theta \approx \theta_0 - \beta\sin\theta_0$: напрямок поширення хвилі зміщується вперед у бік руху джерела на кут $\delta\theta = \beta\sin\theta_0$.
\end{problem}
...
У цьому розділі отримаємо явний розв'язок рівнянь електромагнітного поля за допомогою \emph{4-вектора потенціалу\index{Чотиривектор!потенціалу}}, існування якого забезпечене рівняннями~\eqref{eq:Maxwell_4D_2}: внаслідок цих рівнянь можна ввести коваріантне векторне поле $A_{\mu}$ (4-потенціал), таке, що
...
\begin{equation}\label{eq:wave_eq_A}
    \Dalambertian A^{\nu} = \frac{4\pi}{c}j^{\nu},
\end{equation}
де $\mdlgwhtsquare \equiv \partial_{\nu}\partial^{\nu} = \eta^{\mu\nu}\partial_{\mu}\partial_{\nu}$ --- оператор Даламбера (хвильовий оператор).

Розв'язок~\eqref{eq:wave_eq_A}, що задовольняє також~\eqref{eq:Lorenz_gauge}, породжує розв'язок рівнянь Максвелла для тензора електромагнітного поля за формулою~\eqref{eq:4potential_def}. Далі ми знов звернемося до постановки задачі, що відповідає \emph{ізольованій системі за відсутності зовнішнього випромінювання}. Розглянемо розв'язок~\eqref{eq:wave_eq_A} починаючи з нескінченного минулого, припускаючи, що 4-вектор $j^{\nu}(x)\equiv j^{\nu}(x^0,\vect{r})$ відмінний від нуля лише в обмеженій області просторових координат $\vect{r}$, а будь-які джерела на нескінченності відсутні. Як відомо з курсу математичної фізики, цим умовам відповідає розв'язок рівняння~\eqref{eq:wave_eq_A} у вигляді запізнюючих потенціалів, що можна подати за допомогою фундаментального розв'язку $G_\text{ret}(x)$ оператора Даламбера:
\begin{equation*}
    G_{\text{ret}}(x) = \frac{1}{2\pi}\,\theta(x^0)\,\delta[(x^0)^2 - \vect{r}^2],
    \qquad x\equiv\{x^{\alpha}\}\equiv(x^0,\vect{r}),
\end{equation*}
де $\theta(t)\equiv\{0,\,t<0;\;1,\,t>0\}$ --- функція Хевісайда, $\delta$ --- функція Дірака; цей розв'язок, за визначенням, задовольняє рівнянню
\begin{equation*}
    \Dalambertian G_{\text{ret}}(x) = \delta^4(x)
...
$\int f(x-x')g(x')\,d^4\vect{x}'$; символ <<$*$>> у~\eqref{eq:retarded_potentials} означає саме її.}:
\begin{equation}\label{eq:retarded_potentials}
    A^{\nu} = \frac{4\pi}{c}\,G_{\text{ret}}*j^{\nu}.
\end{equation}
Формально це можна записати так:
\begin{align*}
    A^{\nu}(x) &= \frac{2}{c}\int d^4\vect{x}\;
    \theta(x^0-x'^0)\,\delta[(x^0-x'^0)^2 - (\vect{r}-\vect{r}')^2]\,
    j^{\nu}(x') \\
    &= \frac{1}{c}\int d^3\vect{r}'\;
    \frac{j^{\nu}(x_{\text{ret}}^0,\vect{r}')}{|\vect{r}-\vect{r}'|},
    \qquad x_{\text{ret}}^0 \equiv x^0 - |\vect{r}-\vect{r}'|.
\end{align*}

Розв'язок описує електромагнітне поле обмеженої системи з 4-вектором густини струму $j^{\nu}$. Функція $G_\text{ret}(x)$ є скаляром відносно власних перетворень Лоренца,
оскільки тут аргумент $\delta$-функції залежить від квадрата інтервалу, а в $\theta(x^0)$ аргумент не змінює знак при цих перетвореннях. Тому з~\eqref{eq:retarded_potentials} випливає, що $A^{\nu}$ \textbf{є 4-вектором відносно перетворень власної групи Лоренца\index{Група Лоренца}, оскільки $j^{\nu}$ --- 4-вектор}.

Формула~\eqref{eq:retarded_potentials}, що явно гарантує властивості $A^{\nu}$ як 4-вектора, дозволяє прямо обчислити за допомогою~\eqref{eq:4potential_def} закон перетворення компонент $F_{\mu\nu}$ при переході в іншу систему відліку. Це дає змогу встановити, що $F_{\mu\nu}$ --- \textbf{двічі коваріантний тензор}.
...
\begin{align*}
    ...
    &= \frac{4\pi}{c}\int G_\text{ret}(x-x')\,\frac{\partial}{\partial x'^{\nu}}j^{\nu}(x')\,d^4\vect{x}'
    \equiv \frac{4\pi}{c}\,G_{\text{ret}}*\partial_{\nu}j^{\nu} = 0,
\end{align*}
