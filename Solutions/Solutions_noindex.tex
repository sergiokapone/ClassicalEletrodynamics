...
\begin{multline*}
	...
	\vect{r}') = \\
	= \frac{1}{c} \int dV' \int \delta \left[ t - t' - \frac{|\vect{r} - \vect{r}'|}{c} \right] \frac{\rho (t', \vect{r}')}{|\vect{r} - \vect{r}'|} dt',
\end{multline*}
...
Як зазначено на початку попереднього розділу, задача Коші, на відміну від розгляду ізольованої системи, оперує лише з полями \( t \geqslantt_0 \). Поведінка
джерел та полів за \( t < t_0 \), яка може впливати на стан системи за \( t \geqslantt_0 \), є невідомою. Тому, на відміну від п.~\ref{sec:Potentials_of_isolated_system}, окрім задання функцій
\( \rho(t, \vect{r}) \), \( \vect{j}(t, \vect{r}) \) за \( t \geqslantt_0 \) необхідно задавати певні \emph{початкові умови}.
...
Як видно з наведених міркувань, зміни початкових умов за \( t = 0 \) поза областю~\eqref{eq:2.3.5} не впливають на поле у точці \( \vect{r}_0 \). Це
...
момент, мають задовольнити рівнянням~\eqref{eq:2.3.1} та~\eqref{eq:2.3.2}; тому стан поля та його майбутня еволюція визначаються \emph{чотирма функціями
...
\[
\Delta \tilde\Phi + k^2 \tilde\Phi = 0 \quad \text{в області} \quad r \geqslantR > 0.
\]
