...
джерел та полів за \( t < t_0 \), яка може впливати на стан системи за \( t \geq t_0 \), є невідомою. Тому, на відміну від п.~\ref{sec:Potentials_of_isolated_system}, окрім задання функцій
\( \rho(t, \vect{r}) \), \( \vect{j}(t, \vect{r}) \) за \( t \geq t_0 \) необхідно задавати певні \emph{початкові умови}.
...
\begin{equation*}
    K(t) = \{\vect{r} : |\vect{r} - \vect{r}_0| \leq R(t)\}, \quad \text{де} \quad R(t) = c(T - t), \quad t \leq T.\footnote{Сукупність точок $\{t, \vect{r}\}$ чотиривимірного простору, для яких виконуються ці умови, належать до так званого \emph{конусу минулого} точки $\{0, \vect{r}_0\}$.}
\end{equation*}
...
\frac{dU}{dt} = -c R^2 \int d\Omega \left[ W + \frac1{4\pi}  [\Delta \Efield \times \Delta \Bfield] \cdot \vect{n} \right]\leq 0.
\end{equation}
...
\[
\Delta \tilde\Phi + k^2 \tilde\Phi = 0 \quad \text{в області} \quad r \geq R > 0.
\]
