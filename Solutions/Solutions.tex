...
Рівняння Максвелла є системою восьми скалярних диференціальних рівнянь у частинних похідних
відносно шести компонент полів $\Efield$ та $\Bfield$. Попри те що система є
перевизначеною, безпосереднє розв'язання її в загальному випадку є непростою задачею.
У цьому розділі розглядаються два підходи до побудови розв'язків.

Перший підхід --- метод електромагнітних потенціалів --- зводить вихідну систему до двох
незалежних хвильових рівнянь (рівнянь Даламбера) відносно скалярного $\phi$ та векторного
$\vect{A}$ потенціалів. Вибір зручної \emph{калібрувальної умови} (Лоренца, Гамільтона або
Кулона) дозволяє суттєво спростити структуру рівнянь. Для ізольованої системи зарядів і
струмів розв'язки цих рівнянь набувають вигляду \emph{запізнюючих потенціалів}, що
відображають скінченну швидкість поширення електромагнітних збурень.

Другий підхід --- постановка \emph{задачі Коші} --- дозволяє однозначно визначити еволюцію
електромагнітного поля з заданих початкових умов. Доводиться, що початкові умови, задані на
множині просторових точок, визначають поле лише в межах відповідного <<конуса минулого>> ---
ще один прояв принципу причинності в електродинаміці.
...
\begin{equation*}
	\Efield  + \frac{1}{c} \frac{\partial \vect{A}}{\partial t} = - \nabla \phi,
\end{equation*}
або
...
\begin{equation}
	\Bfield = \Rot \vect{A}', \quad
	\Efield = -\nabla \phi' - \frac{1}{c} \frac{\partial \vect{A}'}{\partial t}.
\end{equation}

Таким чином, потенціали $(\phi, \vect{A})$ містять деякі ступені свободи, що ніяк не
впливають на фізичну ситуацію. Цю обставину називають \emph{калібрувальною інваріантністю} рівнянь поля, а перетворення~\eqref{eq:A'}, \eqref{eq:phi'}, або інші, що не
впливають на спостережувані величини $(\Efield, \Bfield)$, називають \emph{калібрувальними перетвореннями}\index{Калібрувальне перетворення}.
...
потенціали $(\phi, \vect{A})$, за допомогою яких можна звести рівняння до зручнішого вигляду.

Розглянемо умову Лоренца для $(\phi, \vect{A})$:
...
\end{equation*}
Підбираючи функцію $\chi$ таким чином, щоб $f = \Dalambertian  \chi$
...
\begin{equation}\label{eq:Dphi}
	\Dalambertian  \phi = -4\pi \rho.
\end{equation}

Підставимо~\eqref{eq:potB} та~\eqref{eq:potE} в~\eqref{eq:M4D}:
\begin{equation*}
	\Rot(\Rot\vect{A}) = \frac{4\pi}{c}\vect{j}
	+ \frac{1}{c}\left( -\nabla \phi - \frac{1}{c}\frac{\partial \vect{A}}{\partial t} \right).
\end{equation*}
В лівій частині:
\begin{equation*}
	\Rot(\Rot\vect{A}) = \nabla \Div\vect{A} - \nabla^2\vect{A} = -\frac1c \nabla \parttime\phi - \nabla^2\vect{A}.
\end{equation*}
де враховано умову Лоренца~\eqref{eq:LorenzCondition}. Оскільки члени зі скалярним потенціалом
$\phi$ в останній формулі скорочуються, отримуємо:
\begin{equation}\label{eq:DA}
	\Dalambertian  \vect{A} = -\frac{4\pi}{c} \vect{j}.
\end{equation}
...
\begin{equation}
	\Dalambertian \left( \frac1c \parttime{\phi} + \Div\vect{A} \right) = \frac{4\pi}c \left( \parttime{\rho} + \Div\vect{j}\right)  .
\end{equation}
...
виконується, то:
\begin{equation}\label{eq:LorentzCalibr}
	\frac1c \parttime{\phi} + \Div\vect{A} = 0.
\end{equation}

Співвідношення~\eqref{eq:LorentzCalibr}, якщо його розглядати як рівняння для:
\(
...
\begin{equation}\label{eq:Df}
	\Dalambertian f = 0
\end{equation}
не є єдиним (воно має, наприклад, хвильові розв'язки\index{Хвильове рівняння}). Але за умови
...
\begin{equation*}
	\frac{4\pi}c \left[\Div \vect{j} + \frac{\partial \rho}{\partial t}\right] = 0,
\end{equation*}
тобто закон збереження заряду~\eqref{eq:charge_conservation_low_diff} є необхідною умовою розв`язку~\eqref{eq:DA_for_Hamilton1},~\eqref{eq:DA_for_Hamilton2} за умови~\eqref{eq:HamiltonCondition}.
...
\begin{equation}\label{eq:ColumbCondition_DA}
	\Dalambertian \vect{A} = \frac{4\pi}c \vect{j} - \frac1c \nabla \parttime{\phi}.
\end{equation}
...
%% --------------------------------------------------------
\section{Потенціали ізольованої системи зарядів і струмів}\label{sec:Potentials_of_isolated_system}
%% --------------------------------------------------------
...
оперувати з перетворенням Фур'є цих функцій за часом.

Для ізольованої системи буде отримано розв'язок рівнянь~\eqref{eq:Dphi}, \eqref{eq:DA} у вигляді
запізнюючих (загаювальних) потенціалів, який застосовується для розгляду різноманітних задач теорії випромінювання.
...
\begin{equation}\label{eq:tilde_phi}
	\tilde{\phi}(\omega, \vect{r}) = \frac{1}{\sqrt{2\pi}} \int\limits_{-\infty}^{+\infty} dt \, e^{i \omega t} \phi(t, \vect{r}).
\end{equation}
...
вигляд:
\begin{equation}
	\Div \tilde{\vect{A}} - \frac{i \omega}{c} \tilde{\phi} = 0.
\end{equation}
...
\begin{equation}\label{eq:Dphi_Fourier}
	\Delta \tilde{\phi} + k^2 \tilde{\phi} = -4\pi \tilde{\rho}, \quad \text{де} \quad k = \frac{\omega}{c}.
\end{equation}
...
\begin{equation*}
	\Dalambertian \phi = \delta(\vect{r} - \vect{r}_0)\chi(t, \vect{r}_0),
\end{equation*}
де $\chi(t, \vect{r}_0) = 4\pi\rho(t, \vect{r}_0)$. Очевидно, розв'язок~\eqref{eq:Dphi} можна подати, як суперпозицію
таких розв'язків з різними $\vect{r}_0$.

Нехай $\vect{r}_0 = 0$. Поле, що створюється точковим джерелом у точці $\vect{r}$, є сферично-симетричним $\phi = \phi(t, r)$, $r = |\vect{r}|$. Завдяки
сферичній симетрії:
\begin{equation*}
	\Dalambertian \phi = \frac1{c^2}\pparttime{\phi} - \frac1{r^2} \diff{}{r}\left(r^2 \diff{\phi}{r} \right) .
\end{equation*}
Покладемо $\phi = \frac{\psi}{r}$, тоді за $r>0$:
\begin{equation*}
	\Dalambertian \phi = \frac1r \left( \frac1{c^2}\pparttime{\psi} - \ddiff{\psi}{r}\right) =0 .
\end{equation*}
Це одновимірне хвильове рівняння\index{Хвильове рівняння!одновимірне}, яке має загальний розв'язок:
\begin{equation*}
...
випромінюються джерелом, а $f_2$ --- хвилі, що приходять з нескінченності. \emph{За відсутності зовнішнього випромінювання} слід покласти $f_2=0$.
...
Якщо джерело знаходиться у точці $\vect{r}_0 \neq 0$, очевидно:
\begin{equation}\label{eq:rad_phi}
	\phi(t, \vect{r}) = \frac1{|\vect{r} - \vect{r}_0|} f_1\left( t - \frac{|\vect{r} - \vect{r}_0|}c \right).
\end{equation}
...
Для Фур'є-обра\-зів розв'язку~\eqref{eq:rad_phi} маємо:
\begin{equation}
	\tilde{\phi} (\omega, \vect{r}) = \frac{1}{\sqrt{2\pi}} \int dt \, e^{i\omega t} \frac{f_1\left(t - \frac{|\vect{r} - \vect{r}_0|}{c}\right)}{|\vect{r} - \vect{r}_0|}  =
	\frac{\exp \left[ i \frac{\omega}{c} |\vect{r} - \vect{r}_0| \right] }{|\vect{r} - \vect{r}_0|}\tilde{f}_1(\omega),
\end{equation}
де
\(
...
Розв'язок рівняння Гельмгольца, що задовольняє умовам випромінювання, у випадку обмеженої системи джерел має бути суперпозицією розв'язків типу~\eqref{eq:rad_phi_sph} з
різними $\vect{r}_0$ і мати асимптотику
\begin{equation*}
	\tilde{\phi} (\omega, \vect{r}) \approx C(\vect{n}) \frac{e^{i\omega r / c}}{r} + O(r^{-2}),
\end{equation*}
...
безпосередньо,
використовуючи сфе\-рич\-но-симетричний
розв'язок \eqref{eq:rad_phi}. Але ми проведемо аналогічний розгляд з використанням сфе\-рич\-но-симетричного розв'язку рівнянь Гельмгольца, а потім
перейдемо до
розв'язків~\eqref{eq:Dphi}, \eqref{eq:DA} через перетворення Фур'є.

Розглянемо рівняння для скалярного потенціалу~\eqref{eq:Dphi_Fourier} і шукатимемо
фундаментальний розв'язок $\tilde{G}(\vect{r})$
...
домінує доданок
$\Delta \phi$. Тому асимптотика розв'язку за $r \to 0$ повинна збігатися з розв'язком рівняння Пуассона для точкового заряду:
\begin{equation}
	\Delta \left( \frac{q}{r} \right) = -4\pi q \delta (\vect{r}).
\end{equation}

Зіставлення за $r > 0$ дає:
\begin{equation}\label{eq:tildeGr}
	\tilde{G}(r) = - \frac{e^{ikr}}{4\pi r}.
\end{equation}

Більш послідовний розгляд фундаментальних розв'язків операторів Даламбера та Гельмгольца з погляду узагальнених функцій див. додаток~\ref{sec:GeneralizedFunctions}.
...
\begin{equation}\label{eq:tilde_phi_bundle}
	\tilde{\phi} (\omega, \vect{r}) = \int \frac{dV'}{|\vect{r} - \vect{r}'|} \exp \left[ i \frac{\omega}{c} |\vect{r} - \vect{r}'| \right]
...
\begin{equation}
	\phi (t, \vect{r}) = \frac{1}{\sqrt{2\pi}} \int e^{-i\omega t} \tilde{\phi} (\omega, \vect{r}) d\omega,
\end{equation}
\begin{equation}
...
\begin{equation}\label{eq:phi_ret}
	\phi (t, \vect{r}) = \int \frac{dV'}{|\vect{r} - \vect{r}'|} \rho \left( t - \frac{1}{c} |\vect{r} - \vect{r}'|, \vect{r}' \right),
\end{equation}
...
рівнянь~\eqref{eq:Dphi},~\eqref{eq:DA} (див. додаток~\ref{sec:GeneralizedFunctions}). Цей фундаментальний розв'язок має вигляд:
\begin{equation}\label{eq:G}
	G(t, \vect{r}) = \frac{1}{2\pi c} \delta \left( t^2 - r^2 / c^2 \right) \theta(t).
\end{equation}

Згортка з правою частиною~\eqref{eq:Dphi}:
\begin{equation}\label{eq:2.2.18}
	\phi (t, \vect{r}) = 4\pi \int dt' dV' G(t - t', \vect{r} - \vect{r}') \rho (t', \vect{r}').
\end{equation}

Підставимо~\eqref{eq:G}:
\begin{multline*}
	\phi (t, \vect{r}) = \frac2c \int dt' dV' \delta \left[ \left( t - t' \right)^2 - \frac{(\vect{r} - \vect{r}')^2}{c^2} \right] \theta (t - t') \rho (t',
	\vect{r}') = \\
	= \frac{1}{c} \int dV' \int \delta \left[ t - t' - \frac{|\vect{r} - \vect{r}'|}{c} \right] \frac{\rho (t', \vect{r}')}{|t - t'|} dt',
\end{multline*}
...
\begin{multline*}
	\frac1c \frac{\partial \phi}{\partial t} + \Div\vect{A} = \\
	...
	= -\frac{4\pi}{c} \int dt' dV' \left[  \left( \frac{\partial}{\partial t'} G(t - t', \vect{r} - \vect{r}') \right) \rho (t', \vect{r}') + \left(
		\frac{\partial}{\partial x'_i} G(t - t', \vect{r} - \vect{r}') \right) j_i (t', \vect{r}') \right] = \\
	= \frac{4\pi}{c} \int dt' dV' \left[ G(t - t', \vect{r} - \vect{r}') \frac{\partial}{\partial t'} \rho (t', \vect{r}') + G(t - t', \vect{r} -
		\vect{r}') \frac{\partial}{\partial x'_i} j_i (t', \vect{r}') \right] = \\
...
Як зазначено на початку попереднього розділу, задача Коші, на відміну від розгляду ізольованої системи, оперує лише з полями \( t \geqslant t_0 \). Поведінка
джерел та полів за \( t < t_0 \), яка може впливати на стан системи за \( t \geqslant t_0 \), є невідомою. Тому, на відміну від п.~\ref{sec:Potentials_of_isolated_system}, окрім задання функцій
\( \rho(t, \vect{r}) \), \( \vect{j}(t, \vect{r}) \) за \( t \geqslant t_0 \) необхідно задавати певні \emph{початкові умови}.
...
відомий за \( t = 0 \). Розподіли густини заряду \( \rho(t, \vect{r}) \) та густини струму \( \vect{j}(t, \vect{r}) \) за \( t \geqslant0 \) вважаємо
заданими, причому \(\frac{\partial\rho}{\partial t} + \Div\vect{j} = 0\). При \( t = 0 \) задаємо поля \( \Efield \), \( \Bfield \), причому вони мають задовольняти рівняння:
...
З будови системи рівнянь видно, що ми можемо задати електромагнітне поле при \( t = 0 \), яке задовольняє рівнянням~\eqref{eq:2.3.1} та~\eqref{eq:2.3.2}, і розв'язувати потім рівняння~\eqref{eq:2.3.3} та~\eqref{eq:2.3.4} при \( t > 0 \). Однак виникає питання, чи зберігаються при цьому
рівняння~\eqref{eq:2.3.1} та~\eqref{eq:2.3.2} також і при \( t > 0 \)?

Покажемо, що це дійсно так. З рівняння~\eqref{eq:2.3.3} маємо:
...
Звідси видно, що співвідношення~\eqref{eq:2.3.1} справедливо при \( t > 0 \), якщо воно відбувається при \( t = 0 \).

З рівняння~\eqref{eq:2.3.4} маємо:

\begin{equation*}
	\frac{\partial}{\partial t} \Div \Bfield = 0,
\end{equation*}
звідки видно, що рівняння~\eqref{eq:2.3.2} також зберігається при \( t > 0 \).
...
Покажемо, що початкові умови, задані при \( t = 0 \) всередині кулі (див. рис.~\ref{tikz:Unity}):

\begin{equation}
	(x - x_0)^2 + (y - y_0)^2 + (z - z_0)^2 \leqslant c^2 T^2 \label{eq:2.3.5}
\end{equation}
...
умовами в області~\eqref{eq:2.3.5} та розглянемо їхню різницю \( \Delta \Efield = \Efield - \Efield' \), \( \Delta \Bfield = \Bfield - \Bfield'
...
\begin{figure}[ht!]\centering
	\begin{tikzpicture}[scale=1.5]
		...
		\node[right] at (surface) {$t = T$};
		\draw[->] (r0) -- node[above, sloped, pos=0.7] {$\vect{R}(t)$} (surface);
	\end{tikzpicture}
	\caption{}%
	\label{tikz:Unity}
\end{figure}
...
\begin{multline}\label{eq:2.3.6}
	...
	=
	\frac1{4\pi} \left(\Delta\Efield \frac{\partial\Delta\Efield}{\partial t}
	+
	\Delta\Bfield \frac{\partial\Delta\Bfield}{\partial t}
	\right) =
	%
	...
	-\frac{c}{4\pi} \Div [\Delta \Efield \times \Delta
		\Bfield]
\end{multline}
маємо співвідношення, аналогічне закону збереження енергії\index{Закон збереження!енергії} в диференціальній формі:
...
\begin{equation*}
	K(t) = \{\vect{r} : |\vect{r} - \vect{r}_0| \leqslant R(t)\}, \quad \text{де} \quad R(t) = c(T - t), \quad t \leqslant T.\footnote{Сукупність точок $\{t, \vect{r}\}$ чотиривимірного простору, для яких виконуються ці умови, належать до так званого \emph{конуса минулого} точки $\{0, \vect{r}_0\}$.}
\end{equation*}

Перейдемо в інтегралі~\eqref{eq:2.3.7} до сферичних координат з центром у \( \vect{r}_0 \):
...
\begin{multline}\label{eq:2.3.8}
	...
	= -c R^2 \int d\Omega \,  W(t, R(t), \theta, \varphi) - \frac{c}{4\pi} \int_{K(t)} \Div [\Delta \Efield \times \Delta \Bfield] \, dV = \\
	\text{(за формулою Остроградського-Гауса)}\\
	= -c R^2 \int d\Omega \, W - \frac{c}{4\pi} \int\limits_{\partial K(t)} (d\vect{S} \cdot [\Delta \Efield \times \Delta \Bfield]) = \\
...
\begin{equation*}
	| \left[ \Delta \Efield \times \Delta \Bfield \right]  \cdot \vect{n} | \leqslant | \Delta \Efield | \cdot | \Delta \Bfield | \leqslant \frac{\Delta \Efield^2 +
		\Delta \Bfield^2}{2} \leqslant 4\pi W,
\end{equation*}
з~\eqref{eq:2.3.8} випливає:

\begin{equation}\label{eq:2.3.9}
	\frac{dU}{dt} = -c R^2 \int d\Omega \left[ W + \frac1{4\pi}  [\Delta \Efield \times \Delta \Bfield] \cdot \vect{n} \right]\leqslant 0.
\end{equation}
...
\end{equation*}
%
тому \( U(0) = 0 \). За означенням \( U(t) \geqslant0 \), тому нерівність~\eqref{eq:2.3.9} означає, що \( U(t) \equiv 0 \), тобто \( \Efield = \Efield'
\), \( \Bfield = \Bfield' \) у будь-якій кулі \( K(t) \), \( t \in [0, T] \).

Таким чином, розв'язки з однаковими початковими умовами збігаються, тобто ці умови, задані в області~\eqref{eq:2.3.5}, однозначно задають поле у
будь-якій кулі \( K(t) \), \( 0 \leqslant t \leqslant T \).

Як видно з наведених міркувань, зміни початкових умов за \( t = 0 \) поза областю~\eqref{eq:2.3.5} не впливають на поле у точці \( \vect{r}_0 \). Це очевидний наслідок скінченої швидкості поширення взаємодій. Аналогічно, збурення поля, що відбуваються в момент \( t \) поза кулею \( K(t) \), не
...
момент, мають задовольнити рівнянням~\eqref{eq:2.3.1} та~\eqref{eq:2.3.2}; тому стан поля та його майбутня еволюція визначаються \emph{чотирма функціями від трьох просторових змінних}. Наприклад, можна незалежно задати дві компоненти електричного поля та дві --- магнітного.
...
%=========================================================
\begin{problem}%
Вектор-потенціал стаціонарного магнітного поля обмеженої системи струмів на великих відстанях можна наближено подати як \( \vect{A}(\vect{r}) = \frac{1}{cr^{3}} \int dV\, \vect{j}(\vect{r}')(\vect{r}\cdot\vect{r}') \). Записати цей вираз через магнітний момент \( \vect{m} = \frac{1}{2c}\int dV\,[\vect{r}\times\vect{j}] \). Відповідь: \( \vect{A}(\vect{r}) = [\vect{m} \times \vect{r}] / r^3 \).
\end{problem}


%=========================================================
\begin{problem}%
Знайти загальний сферично симетричний розв'язок  \( \tilde\phi \equiv \tilde\phi(k, r) \) рівняння Гельмгольца в вакуумі:
\[
	\Delta \tilde\phi + k^2 \tilde\phi = 0 \quad \text{в області} \quad r \geqslant R > 0.
\]
...
%=========================================================
\begin{problem}%
Знайти розв'язок \( \tilde\phi \equiv \tilde\phi(k, r) \) рівняння Гельмгольца:
\begin{equation*}
	\Delta \tilde\phi + k^2 \tilde\phi = -4\pi\tilde\rho
\end{equation*}
у випадку сферичної симетрії \(
\tilde\rho \equiv \tilde\rho(k, r) \), за умови відсутності зовнішнього випромінювання. Функція \( \tilde\rho \equiv \tilde\rho(k, r) \) відмінна від нуля лише в області \( r <
R \). Розв'язок подати у вигляді квадратур. Показати, що при \( R \to 0 \) за умови фіксованого \( \int dV \, \tilde\rho(k, r) = 4\pi \int_0^R dr \, r^2
\tilde\rho(k, r) = q \), матимемо \( \tilde\Phi(k, r) = q \exp(ikr) / r \).
\end{problem}
