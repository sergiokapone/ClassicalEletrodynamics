...
Існує досить багато методів аналізу рівнянь Максвелла~\eqref{eq:M1D} --~\eqref{eq:M4D}, серед яких одним з найбільш поширених є введення потенціалів\index{Електромагнітні потенціали}, що
дозволяють дещо
зменшити число невідомих функцій.

З умови соленоїдальності\index{Соленоїдальність} магнітного поля~\eqref{eq:M2D} випливає, що існує деяке векторне поле $\vect{A}$\index{Вектор-потенціал}, таке, що:
...
\end{equation*}
звідки випливає існування скалярного поля $\phi$\index{Скалярний потенціал}, такого, що
\begin{equation*}
...
впливають на фізичну ситуацію. Цю обставину називають \emph{калібрувальною інваріантністю}\index{Калібрувальна інваріантність} рівнянь поля, а перетворення~\eqref{eq:A'},~\eqref{eq:phi'}, або інші, що не
...
%% --------------------------------------------------------
\subsection*{Калібрувальна умова Лоренца\index{Калібрувальна умова!Лоренца}}
%% --------------------------------------------------------
...
(де $\mdlgwhtsquare\  = \nabla^2 - \frac{1}{c^2} \frac{\partial^2}{\partial t^2}$ --- оператор Даламбера\index{Оператор Даламбера}), бачимо, що нові потенціали $(\phi',
	\vect{A}')$
...
\begin{equation}\label{eq:LorentzCalibr}
	\left( \frac1c \parttime{\phi} + \Div\vect{A} \right) = 0.
\end{equation}
...
система зарядів і струмів, рівя~\eqref{eq:Df} має тільки тривіальний розв'язок
$f=0$. Ця ситуація відповідає запізнюючим розв'язкам рівнянняннь~\eqref{eq:Dphi} та~\eqref{eq:DA}, що
розглядаються нижче.



%% --------------------------------------------------------
\subsection*{Калібрування Гамільтона\index{Калібрувальна умова!Гамільтона}}
%% --------------------------------------------------------
...
%% --------------------------------------------------------
\subsection*{Калібрування Кулона\index{Калібрувальна умова!Кулона}}
%% --------------------------------------------------------
...
За умовою~\eqref{eq:ColumbCondition} з~\eqref{eq:M1D} маємо рівняння Пуассона\index{Рівняння Пуассона} для потенціалу~$\phi$:
...
%% --------------------------------------------------------
\subsection*{Перетворення Фур’є\index{Перетворення Фур’є} та рівняння Гельмгольца\index{Рівняння Гельмгольца}}
%% --------------------------------------------------------
...
%% --------------------------------------------------------
\subsection*{Умова випромінювання\index{Умова випромінювання} для ізольованої системи}
%% --------------------------------------------------------


Систему зарядів і струмів називатимемо ізольованою\index{Ізольована система}, якщо вона зосереджена в обмеженій області за відсутності зовнішнього випромінювання, що йде з
...
Нехай $\vect{r}_0 = 0$. Поле, що створюється точковим джерелом у точці $\vect{r}$, є сферично-симетричним\index{Сферична симетрія} $\phi = \phi(t, r)$, $r = |\vect{r}|$. Завдяки
сферичній симетрії:
\begin{equation*}
...
%% --------------------------------------------------------
\subsection*{Запізнюючі потенціали\index{Запізнюючі потенціали}}
%% --------------------------------------------------------
...
фундаментальний розв’язок\index{Фундаментальний розв’язок} $\tilde{G}(\vect{r})$
...
Більш послідовний розгляд фундаментальних розв’язків операторів Даламбера та Гельмгольца з точки зору узагальнених функцій див. додаток~\ref{sec:GeneralizedFunctions}.
...
\end{equation}
де $t_{\text{ret}} = t - \frac{1}{c} |\vect{r} - \vect{r}'|$\index{Запізнюючий час}.
...
Зауважимо, що~\eqref{eq:phi_ret},~\eqref{eq:A_ret} можна записати у вигляді згортки фундаментального розв’язку оператора Даламбера\index{Оператор Даламбера!фундаментальний розв’язок} з правими частинами
рівнянь
\eqref{eq:Dphi},
\eqref{eq:DA} (див. додаток~\ref{sec:GeneralizedFunctions}). Цей фундаментальний розв’язок має вигляд:
...
\( \rho(t, \vect{r}) \), \( \vect{j}(t, \vect{r}) \) за \( t \geqslant t_0 \) необхідно задавати певні \emph{початкові умови}\index{Початкові умови}.
...
%% --------------------------------------------------------
\subsection*{Єдиність розв’язків\index{Єдиність розв’язків}}
%% --------------------------------------------------------
...
\begin{equation*}
    K(t) = \{\vect{r} : |\vect{r} - \vect{r}_0| \leqslant R(t)\}, \quad \text{де} \quad R(t) = c(T - t), \quad t \leqslant T.\footnote{Сукупність точок $\{t, \vect{r}\}$ чотиривимірного простору, для яких виконуються ці умови, належать до так званого \emph{конусу минулого} точки $\{0, \vect{r}_0\}$.}
\end{equation*}
...
Таким чином, розв’язки з однаковими початковими умовами співпадають, тобто ці умови, задані в області~\eqref{eq:2.3.5}, однозначно задають поле у
будь-якій кулі \( K(t) \), \( 0 \leqslant t \leqslant T \).

Як видно з наведених міркувань, зміни початкових умов за \( t = 0 \) поза областю~\eqref{eq:2.3.5} не впливають на поле у точці \( \vect{r}_0 \). Це очевидний наслідок скінченої швидкості поширення взаємодій\index{Швидкість світла!скінченість}. Аналогічно, збурення поля, що відбуваються в момент \( t \) поза кулею \( K(t) \), не
...
момент, мають задовольнити рівнянням~\eqref{eq:2.3.1} та~\eqref{eq:2.3.2}; тому стан поля та його майбутня еволюція визначаються \emph{чотирма функціями від трьох просторових змінних}\index{Ступені свободи!електромагнітного поля}. Наприклад, можна незалежно задати дві компоненти електричного поля та дві --- магнітного.
...
c^{-1} r^{-3} \int dV \, \vect{j}(\vect{r}')(\vect{r} \cdot \vect{r}') \). Записати цей вираз через магнітний момент\index{Магнітний момент} \( \vect{m} = (2c)^{-1} \int dV \,
